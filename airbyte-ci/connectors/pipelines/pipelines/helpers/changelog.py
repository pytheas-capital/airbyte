...
class Changelog:
    ...
    def to_markdown(self) -> str:
        ...
        new_lines = self.original_markdown_lines[:self.changelog_entries_start_line_index] + [l.to_str() for l in sorted_entries] + self.original_markdown_lines[self.last_changelog_line_index:]
        return "\n".join(new_lines) + "\n"

    def __parse_markdown(self) -> int:
        changelog_entry_re=(codecs.decode(r"^\| *(?P<version>[0-9]+\.[0-9+]+\.[0-9]+?) *\| *", 'unicode_escape') +
            codecs.decode(r"(?P<day>[0-9]{4}-[0-9]{2}-[0-9]{2}) *\| *", 'unicode_escape') +
            codecs.decode(r"\[?(?P<pr_number1>[0-9]*)\]? ?\(https://github.com/airbytehq/airbyte/pull/(?P<pr_number2>[0-9]*)\) *\| *", 'unicode_escape') +
        ...
        for line_index, line in changelog_line_enumerator:
            if re.search(fr'\| *Version *\| *Date *\| *Pull Request *\| *Subject *\|', line):
