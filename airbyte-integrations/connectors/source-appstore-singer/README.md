...
```bash
airbyte-ci connectors --name source-appstore-singer build
```
Once the command is done, you will find your connector image in your local docker registry: `airbyte/source-appstore-singer:dev`.
...
```Dockerfile
FROM airbyte/source-appstore-singer:latest
...
docker run airbyte/source-appstore-singer:dev spec
```
#### Run
